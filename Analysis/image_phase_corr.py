...
import numpy as np
from .fft_import import fftmodule as fft
from .image_masked_phase_corr import _masked_phase_cross_correlation
...
def phase_cross_correlation(reference_image, moving_image,
    ...
    if (reference_mask is not None) or (moving_mask is not None):
        if space == 'fourier':
            raise ValueError('For masked registration, only real images are allowed')
        masked = True
        shifts = _masked_phase_cross_correlation(reference_image, moving_image,
    ...
    if upsample_factor == 1:
        if return_error and not masked:
    ...
    # If upsampling > 1, then refine estimate with matrix multiply DFT
        ...
        if 'image_product' not in locals():
            # we are calling masked operation
            src_freq = fft.fftn(reference_image)
            target_freq = fft.fftn(moving_image)
            shape = src_freq.shape
            image_product = src_freq * target_freq.conj()
        ...
        if not masked and return_error:
            src_amp = np.sum(np.real(src_freq * src_freq.conj()))
            target_amp = np.sum(np.real(target_freq * target_freq.conj()))
